...
HRESULT CWall::Ready_GameObject_Prototype() // 원본객체의 초기화.
{

	return S_OK;
}

HRESULT CWall::Ready_GameObject(void* pArg) // 복제된객체의 추가 초기화.
{
...
_int CWall::LateUpdate_GameObject(_double TimeDelta)
{
...
HRESULT CWall::Add_Component()
{
	...
	// For.Com_Mesh
	if (FAILED(CGameObject::Add_Component(SCENE_STAGE, L"Component_Mesh_Wall", L"Com_Mesh", (CComponent**)&m_pMeshCom)))
		return E_FAIL;
...
HRESULT CWall::SetUp_ConstantTable()
{
	if (nullptr == m_pShaderCom || 
...
CWall * CWall::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	...
	{
		MSG_BOX("Failed while Creating CWall");
