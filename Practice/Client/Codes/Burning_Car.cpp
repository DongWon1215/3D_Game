...
#include "..\Headers\Burning_Car.h"

USING(Client)

CBurning_Car::CBurning_Car(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CGameObject(pGraphic_Device)
{

}

CBurning_Car::CBurning_Car(const CBurning_Car & rhs)
	: CGameObject(rhs)
{

}

HRESULT CBurning_Car::Ready_GameObject_Prototype() // 원본객체의 초기화.
{

	return S_OK;
}

HRESULT CBurning_Car::Ready_GameObject(void* pArg) // 복제된객체의 추가 초기화.
{
...
_int CBurning_Car::Update_GameObject(_double TimeDelta)
{
...
_int CBurning_Car::LateUpdate_GameObject(_double TimeDelta)
{
...
HRESULT CBurning_Car::Render_GameObject()
{
...
HRESULT CBurning_Car::Add_Component()
{
	// For.Com_Shader
	...
	// For.Com_Mesh
	if (FAILED(CGameObject::Add_Component(SCENE_STAGE, L"Component_Mesh_Burning_Car", L"Com_Mesh", (CComponent**)&m_pMeshCom)))
		return E_FAIL;
...
HRESULT CBurning_Car::SetUp_ConstantTable()
{
	if (nullptr == m_pShaderCom || 
...
HRESULT CBurning_Car::SetUp_OnTerrain()
{
	CManagement*	pManagement = CManagement::GetInstance();
	if (nullptr == pManagement)
		return -1;
...
CBurning_Car * CBurning_Car::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CBurning_Car* pInstance = new CBurning_Car(pGraphic_Device);

	if (FAILED(pInstance->Ready_GameObject_Prototype()))
	{
		MSG_BOX("Failed while Creating CBurning_Car");
...
CGameObject * CBurning_Car::Clone_GameObject(void* pArg)
{
	CBurning_Car* pInstance = new CBurning_Car(*this);
