...
#include "..\Headers\ManHole.h"

USING(Client)

CManHole::CManHole(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CGameObject(pGraphic_Device)
{

}

CManHole::CManHole(const CManHole & rhs)
	: CGameObject(rhs)
{

}

HRESULT CManHole::Ready_GameObject_Prototype() // 원본객체의 초기화.
{

	return S_OK;
}

HRESULT CManHole::Ready_GameObject(void* pArg) // 복제된객체의 추가 초기화.
{
...
_int CManHole::Update_GameObject(_double TimeDelta)
{
...
_int CManHole::LateUpdate_GameObject(_double TimeDelta)
{
...
HRESULT CManHole::Render_GameObject()
{
...
HRESULT CManHole::Add_Component()
{
	// For.Com_Shader
	...
	// For.Com_Mesh
	if (FAILED(CGameObject::Add_Component(SCENE_STAGE, L"Component_Mesh_ManHole", L"Com_Mesh", (CComponent**)&m_pMeshCom)))
		return E_FAIL;
...
HRESULT CManHole::SetUp_ConstantTable()
{
	if (nullptr == m_pShaderCom || 
...
HRESULT CManHole::SetUp_OnTerrain()
{
	CManagement*	pManagement = CManagement::GetInstance();
	if (nullptr == pManagement)
		return -1;
...
CManHole * CManHole::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CManHole* pInstance = new CManHole(pGraphic_Device);

	if (FAILED(pInstance->Ready_GameObject_Prototype()))
	{
		MSG_BOX("Failed while Creating CManHole");
...
CGameObject * CManHole::Clone_GameObject(void* pArg)
{
	CManHole* pInstance = new CManHole(*this);
