...
#include "..\Headers\Lighting_Street.h"

USING(Client)

CLighting_Street::CLighting_Street(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CGameObject(pGraphic_Device)
{

}

CLighting_Street::CLighting_Street(const CLighting_Street & rhs)
	: CGameObject(rhs)
{

}

HRESULT CLighting_Street::Ready_GameObject_Prototype() // 원본객체의 초기화.
{

	return S_OK;
}

HRESULT CLighting_Street::Ready_GameObject(void* pArg) // 복제된객체의 추가 초기화.
{
...
_int CLighting_Street::Update_GameObject(_double TimeDelta)
{
...
_int CLighting_Street::LateUpdate_GameObject(_double TimeDelta)
{
...
HRESULT CLighting_Street::Render_GameObject()
{
...
HRESULT CLighting_Street::Add_Component()
{
	// For.Com_Shader
	...
	// For.Com_Mesh
	if (FAILED(CGameObject::Add_Component(SCENE_STAGE, L"Component_Mesh_Lighting_Street", L"Com_Mesh", (CComponent**)&m_pMeshCom)))
		return E_FAIL;
...
HRESULT CLighting_Street::SetUp_ConstantTable()
{
	if (nullptr == m_pShaderCom || 
...
HRESULT CLighting_Street::SetUp_OnTerrain()
{
	CManagement*	pManagement = CManagement::GetInstance();
	if (nullptr == pManagement)
		return -1;
...
CLighting_Street * CLighting_Street::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CLighting_Street* pInstance = new CLighting_Street(pGraphic_Device);

	if (FAILED(pInstance->Ready_GameObject_Prototype()))
	{
		MSG_BOX("Failed while Creating CLighting_Street");
...
CGameObject * CLighting_Street::Clone_GameObject(void* pArg)
{
	CLighting_Street* pInstance = new CLighting_Street(*this);
