...
HRESULT CHouse::Ready_GameObject_Prototype() // 원본객체의 초기화.
{

	return S_OK;
}

HRESULT CHouse::Ready_GameObject(void* pArg) // 복제된객체의 추가 초기화.
{
...
_int CHouse::Update_GameObject(_double TimeDelta)
{
...
_int CHouse::LateUpdate_GameObject(_double TimeDelta)
{
...
HRESULT CHouse::Render_GameObject()
{
...
HRESULT CHouse::Add_Component()
{
	...
	// For.Com_Mesh
	if (FAILED(CGameObject::Add_Component(SCENE_STAGE, L"Component_Mesh_House", L"Com_Mesh", (CComponent**)&m_pMeshCom)))
		return E_FAIL;
...
HRESULT CHouse::SetUp_ConstantTable()
{
	if (nullptr == m_pShaderCom || 
...
HRESULT CHouse::SetUp_OnTerrain()
{
	CManagement*	pManagement = CManagement::GetInstance();
	if (nullptr == pManagement)
		return -1;
...
CHouse * CHouse::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	...
	{
		MSG_BOX("Failed while Creating CHouse");
