...
CSign::CSign(LPDIRECT3DDEVICE9 pGraphic_Device)
...
HRESULT CSign::Ready_GameObject_Prototype() // 원본객체의 초기화.
{

	return S_OK;
}

HRESULT CSign::Ready_GameObject(void* pArg) // 복제된객체의 추가 초기화.
{
...
_int CSign::Update_GameObject(_double TimeDelta)
{
...
_int CSign::LateUpdate_GameObject(_double TimeDelta)
{
...
HRESULT CSign::Render_GameObject()
{
...
HRESULT CSign::Add_Component()
{
	...
	// For.Com_Mesh
	if (FAILED(CGameObject::Add_Component(SCENE_STAGE, L"Component_Mesh_Sign", L"Com_Mesh", (CComponent**)&m_pMeshCom)))
		return E_FAIL;
...
HRESULT CSign::SetUp_ConstantTable()
{
	if (nullptr == m_pShaderCom || 
...
CSign * CSign::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CSign* pInstance = new CSign(pGraphic_Device);

	if (FAILED(pInstance->Ready_GameObject_Prototype()))
	{
		MSG_BOX("Failed while Creating CSign");
...
CGameObject * CSign::Clone_GameObject(void* pArg)
{
	CSign* pInstance = new CSign(*this);
