...
HRESULT CSniper::Ready_GameObject(void * pArg)
{
...
_int CSniper::Update_GameObject(_double TimeDelta)
{
	CManagement* pManagement = CManagement::GetInstance();
	if (nullptr == pManagement)
		return -1;
...
_int CSniper::LateUpdate_GameObject(_double TimeDelta)
{
	if (m_eWeapon == WEAPON_SNIPER)
	{
		...
		if (pManagement->KeyPressing(KEY_LBUTTON) && m_iCurrentBullet > 0.f && !m_bIsFrameStart && m_fCoolTime <= 0.f)
		{
			m_pMeshCom->SetUp_AnimationSet(0);
		...
		else if (dynamic_cast<CPlayer*>(pManagement->Get_GameObject(L"GameObject_Player", SCENE_STAGE, L"Layer_Player", 0))->Get_PlayerState() == STATE_RELOAD || m_iCurrentBullet <= 0)
		{
...
HRESULT CSniper::Render_GameObject()
{
...
HRESULT CSniper::Add_Component()
{
	...
	// For.Com_Mesh
	if (FAILED(CGameObject::Add_Component(SCENE_STAGE, L"Component_Mesh_Sniper", L"Com_Mesh", (CComponent**)&m_pMeshCom)))
		return E_FAIL;
...
HRESULT CSniper::SetUp_ConstantTable()
{
	if (nullptr == m_pShaderCom ||
...
CSniper * CSniper::Create(LPDIRECT3DDEVICE9 pDevice)
{
	...
	{
		MSG_BOX("Failed while Creating CSniper");
...
CGameObject * CSniper::Clone_GameObject(void * pArg)
{
	CSniper* pInstance = new CSniper(*this);

	if (FAILED(pInstance->Ready_GameObject(pArg)))
	{
		MSG_BOX("Failed while Creating CSniper_Clone");
		Safe_Release(pInstance);
	}
