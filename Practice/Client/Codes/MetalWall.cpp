...
#include "..\Headers\MetalWall.h"

USING(Client)

CMetalWall::CMetalWall(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CGameObject(pGraphic_Device)
{

}

CMetalWall::CMetalWall(const CMetalWall & rhs)
	: CGameObject(rhs)
{

}

HRESULT CMetalWall::Ready_GameObject_Prototype() // 원본객체의 초기화.
{

	return S_OK;
}

HRESULT CMetalWall::Ready_GameObject(void* pArg) // 복제된객체의 추가 초기화.
{
...
_int CMetalWall::Update_GameObject(_double TimeDelta)
{
...
_int CMetalWall::LateUpdate_GameObject(_double TimeDelta)
{
...
HRESULT CMetalWall::Render_GameObject()
{
...
HRESULT CMetalWall::Add_Component()
{
	// For.Com_Shader
	...
	// For.Com_Mesh
	if (FAILED(CGameObject::Add_Component(SCENE_STAGE, L"Component_Mesh_MetalWall", L"Com_Mesh", (CComponent**)&m_pMeshCom)))
		return E_FAIL;
...
HRESULT CMetalWall::SetUp_ConstantTable()
{
	if (nullptr == m_pShaderCom || 
...
HRESULT CMetalWall::SetUp_OnTerrain()
{
	CManagement*	pManagement = CManagement::GetInstance();
	if (nullptr == pManagement)
		return -1;
...
CMetalWall * CMetalWall::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CMetalWall* pInstance = new CMetalWall(pGraphic_Device);

	if (FAILED(pInstance->Ready_GameObject_Prototype()))
	{
		MSG_BOX("Failed while Creating CMetalWall");
...
CGameObject * CMetalWall::Clone_GameObject(void* pArg)
{
	CMetalWall* pInstance = new CMetalWall(*this);
