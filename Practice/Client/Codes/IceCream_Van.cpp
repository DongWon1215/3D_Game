...
#include "..\Headers\IceCream_Van.h"

USING(Client)

CIceCream_Van::CIceCream_Van(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CGameObject(pGraphic_Device)
{

}

CIceCream_Van::CIceCream_Van(const CIceCream_Van & rhs)
	: CGameObject(rhs)
{

}

HRESULT CIceCream_Van::Ready_GameObject_Prototype() // 원본객체의 초기화.
{

	return S_OK;
}

HRESULT CIceCream_Van::Ready_GameObject(void* pArg) // 복제된객체의 추가 초기화.
{
...
_int CIceCream_Van::Update_GameObject(_double TimeDelta)
{
...
_int CIceCream_Van::LateUpdate_GameObject(_double TimeDelta)
{
...
HRESULT CIceCream_Van::Render_GameObject()
{
...
HRESULT CIceCream_Van::Add_Component()
{
	// For.Com_Shader
	...
	// For.Com_Mesh
	if (FAILED(CGameObject::Add_Component(SCENE_STAGE, L"Component_Mesh_IceCream_Van", L"Com_Mesh", (CComponent**)&m_pMeshCom)))
		return E_FAIL;
...
HRESULT CIceCream_Van::SetUp_ConstantTable()
{
	if (nullptr == m_pShaderCom || 
...
HRESULT CIceCream_Van::SetUp_OnTerrain()
{
	CManagement*	pManagement = CManagement::GetInstance();
	if (nullptr == pManagement)
		return -1;
...
CIceCream_Van * CIceCream_Van::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CIceCream_Van* pInstance = new CIceCream_Van(pGraphic_Device);

	if (FAILED(pInstance->Ready_GameObject_Prototype()))
	{
		MSG_BOX("Failed while Creating CIceCream_Van");
...
CGameObject * CIceCream_Van::Clone_GameObject(void* pArg)
{
	CIceCream_Van* pInstance = new CIceCream_Van(*this);
