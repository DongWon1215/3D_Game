...
#include "..\Headers\PrisonWall_Small.h"

USING(Client)

CPrisonWall_Small::CPrisonWall_Small(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CGameObject(pGraphic_Device)
{

}

CPrisonWall_Small::CPrisonWall_Small(const CPrisonWall_Small & rhs)
	: CGameObject(rhs)
{

}

HRESULT CPrisonWall_Small::Ready_GameObject_Prototype() // 원본객체의 초기화.
{

	return S_OK;
}

HRESULT CPrisonWall_Small::Ready_GameObject(void* pArg) // 복제된객체의 추가 초기화.
{
...
_int CPrisonWall_Small::Update_GameObject(_double TimeDelta)
{
...
_int CPrisonWall_Small::LateUpdate_GameObject(_double TimeDelta)
{
...
HRESULT CPrisonWall_Small::Render_GameObject()
{
...
HRESULT CPrisonWall_Small::Add_Component()
{
	// For.Com_Shader
	...
	// For.Com_Mesh
	if (FAILED(CGameObject::Add_Component(SCENE_STAGE, L"Component_Mesh_PrisonWall_Small", L"Com_Mesh", (CComponent**)&m_pMeshCom)))
		return E_FAIL;
...
HRESULT CPrisonWall_Small::SetUp_ConstantTable()
{
	if (nullptr == m_pShaderCom || 
...
HRESULT CPrisonWall_Small::SetUp_OnTerrain()
{
	CManagement*	pManagement = CManagement::GetInstance();
	if (nullptr == pManagement)
		return -1;
...
CPrisonWall_Small * CPrisonWall_Small::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CPrisonWall_Small* pInstance = new CPrisonWall_Small(pGraphic_Device);

	if (FAILED(pInstance->Ready_GameObject_Prototype()))
	{
		MSG_BOX("Failed while Creating CPrisonWall_Small");
...
CGameObject * CPrisonWall_Small::Clone_GameObject(void* pArg)
{
	CPrisonWall_Small* pInstance = new CPrisonWall_Small(*this);
