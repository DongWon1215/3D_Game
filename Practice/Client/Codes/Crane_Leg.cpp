...
#include "..\Headers\Crane_Leg.h"

USING(Client)

CCrane_Leg::CCrane_Leg(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CGameObject(pGraphic_Device)
{

}

CCrane_Leg::CCrane_Leg(const CCrane_Leg & rhs)
	: CGameObject(rhs)
{

}

HRESULT CCrane_Leg::Ready_GameObject_Prototype() // 원본객체의 초기화.
{

	return S_OK;
}

HRESULT CCrane_Leg::Ready_GameObject(void* pArg) // 복제된객체의 추가 초기화.
{
...
_int CCrane_Leg::Update_GameObject(_double TimeDelta)
{
...
_int CCrane_Leg::LateUpdate_GameObject(_double TimeDelta)
{
...
HRESULT CCrane_Leg::Render_GameObject()
{
...
HRESULT CCrane_Leg::Add_Component()
{
	// For.Com_Shader
	...
	// For.Com_Mesh
	if (FAILED(CGameObject::Add_Component(SCENE_STAGE, L"Component_Mesh_Crane_Leg", L"Com_Mesh", (CComponent**)&m_pMeshCom)))
		return E_FAIL;
...
HRESULT CCrane_Leg::SetUp_ConstantTable()
{
	if (nullptr == m_pShaderCom || 
...
HRESULT CCrane_Leg::SetUp_OnTerrain()
{
	CManagement*	pManagement = CManagement::GetInstance();
	if (nullptr == pManagement)
		return -1;
...
CCrane_Leg * CCrane_Leg::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CCrane_Leg* pInstance = new CCrane_Leg(pGraphic_Device);

	if (FAILED(pInstance->Ready_GameObject_Prototype()))
	{
		MSG_BOX("Failed while Creating CCrane_Leg");
...
CGameObject * CCrane_Leg::Clone_GameObject(void* pArg)
{
	CCrane_Leg* pInstance = new CCrane_Leg(*this);
