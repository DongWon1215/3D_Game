...
HRESULT CBox::Ready_GameObject_Prototype() // 원본객체의 초기화.
{

	return S_OK;
}

HRESULT CBox::Ready_GameObject(void* pArg) // 복제된객체의 추가 초기화.
{
...
_int CBox::Update_GameObject(_double TimeDelta)
{
...
_int CBox::LateUpdate_GameObject(_double TimeDelta)
{
...
HRESULT CBox::Render_GameObject()
{
...
HRESULT CBox::SetUp_ConstantTable()
{
	if (nullptr == m_pShaderCom ||
...
CBox * CBox::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	...
	{
		MSG_BOX("Failed while Creating CBox");
...
CGameObject * CBox::Clone_GameObject(void* pArg)
{
