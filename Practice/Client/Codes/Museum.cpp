...
#include "..\Headers\Museum.h"

USING(Client)

CMuseum::CMuseum(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CGameObject(pGraphic_Device)
{

}

CMuseum::CMuseum(const CMuseum & rhs)
	: CGameObject(rhs)
{

}

HRESULT CMuseum::Ready_GameObject_Prototype() // 원본객체의 초기화.
{

	return S_OK;
}

HRESULT CMuseum::Ready_GameObject(void* pArg) // 복제된객체의 추가 초기화.
{
...
_int CMuseum::Update_GameObject(_double TimeDelta)
{
...
_int CMuseum::LateUpdate_GameObject(_double TimeDelta)
{
...
HRESULT CMuseum::Render_GameObject()
{
...
HRESULT CMuseum::Add_Component()
{
	...
	// For.Com_Mesh
	if (FAILED(CGameObject::Add_Component(SCENE_STAGE, L"Component_Mesh_Museum", L"Com_Mesh", (CComponent**)&m_pMeshCom)))
		return E_FAIL;
...
HRESULT CMuseum::SetUp_ConstantTable()
{
	if (nullptr == m_pShaderCom || 
...
HRESULT CMuseum::SetUp_OnTerrain()
{
	CManagement*	pManagement = CManagement::GetInstance();
	if (nullptr == pManagement)
		return -1;
...
CMuseum * CMuseum::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CMuseum* pInstance = new CMuseum(pGraphic_Device);

	if (FAILED(pInstance->Ready_GameObject_Prototype()))
	{
		MSG_BOX("Failed while Creating CMuseum");
...
CGameObject * CMuseum::Clone_GameObject(void* pArg)
{
	CMuseum* pInstance = new CMuseum(*this);
