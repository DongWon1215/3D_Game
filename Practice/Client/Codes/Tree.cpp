...
HRESULT CTree::Ready_GameObject_Prototype() // 원본객체의 초기화.
{

	return S_OK;
}

HRESULT CTree::Ready_GameObject(void* pArg) // 복제된객체의 추가 초기화.
{
...
_int CTree::Update_GameObject(_double TimeDelta)
{
...
_int CTree::LateUpdate_GameObject(_double TimeDelta)
{
...
HRESULT CTree::Add_Component()
{
	...
	// For.Com_Mesh
	if (FAILED(CGameObject::Add_Component(SCENE_STAGE, L"Component_Mesh_Tree", L"Com_Mesh", (CComponent**)&m_pMeshCom)))
		return E_FAIL;
...
CTree * CTree::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	...
	{
		MSG_BOX("Failed while Creating CTree");
