...
#include "..\Headers\House3.h"

USING(Client)

CHouse3::CHouse3(LPDIRECT3DDEVICE9 pGraphic_Device)
...
HRESULT CHouse3::Ready_GameObject_Prototype() // 원본객체의 초기화.
{

	return S_OK;
}

HRESULT CHouse3::Ready_GameObject(void* pArg) // 복제된객체의 추가 초기화.
{
...
_int CHouse3::Update_GameObject(_double TimeDelta)
{
...
_int CHouse3::LateUpdate_GameObject(_double TimeDelta)
{
...
HRESULT CHouse3::Render_GameObject()
{
...
HRESULT CHouse3::Add_Component()
{
	...
	// For.Com_Mesh
	if (FAILED(CGameObject::Add_Component(SCENE_STAGE, L"Component_Mesh_House3", L"Com_Mesh", (CComponent**)&m_pMeshCom)))
		return E_FAIL;
...
HRESULT CHouse3::SetUp_ConstantTable()
{
	if (nullptr == m_pShaderCom || 
...
HRESULT CHouse3::SetUp_OnTerrain()
{
	CManagement*	pManagement = CManagement::GetInstance();
	if (nullptr == pManagement)
		return -1;
...
CHouse3 * CHouse3::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CHouse3* pInstance = new CHouse3(pGraphic_Device);

	if (FAILED(pInstance->Ready_GameObject_Prototype()))
	{
		MSG_BOX("Failed while Creating CHouse3");
...
CGameObject * CHouse3::Clone_GameObject(void* pArg)
{
