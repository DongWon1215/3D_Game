...
HRESULT CFence::Ready_GameObject_Prototype() // 원본객체의 초기화.
{

	return S_OK;
}

HRESULT CFence::Ready_GameObject(void* pArg) // 복제된객체의 추가 초기화.
{
...
_int CFence::Update_GameObject(_double TimeDelta)
{
...
_int CFence::LateUpdate_GameObject(_double TimeDelta)
{
...
HRESULT CFence::Render_GameObject()
{
...
HRESULT CFence::Add_Component()
{
	...
	// For.Com_Mesh
	if (FAILED(CGameObject::Add_Component(SCENE_STAGE, L"Component_Mesh_Fence", L"Com_Mesh", (CComponent**)&m_pMeshCom)))
		return E_FAIL;
...
HRESULT CFence::SetUp_ConstantTable()
{
	if (nullptr == m_pShaderCom || 
...
HRESULT CFence::SetUp_OnTerrain()
{
	CManagement*	pManagement = CManagement::GetInstance();
	if (nullptr == pManagement)
		return -1;
...
CFence * CFence::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	...
	{
		MSG_BOX("Failed while Creating CFence");
