...
#include "..\Headers\House5.h"

USING(Client)

CHouse5::CHouse5(LPDIRECT3DDEVICE9 pGraphic_Device)
...
HRESULT CHouse5::Ready_GameObject_Prototype() // 원본객체의 초기화.
{

	return S_OK;
}

HRESULT CHouse5::Ready_GameObject(void* pArg) // 복제된객체의 추가 초기화.
{
...
_int CHouse5::Update_GameObject(_double TimeDelta)
{
...
_int CHouse5::LateUpdate_GameObject(_double TimeDelta)
{
...
HRESULT CHouse5::Render_GameObject()
{
...
HRESULT CHouse5::Add_Component()
{
	...
	// For.Com_Mesh
	if (FAILED(CGameObject::Add_Component(SCENE_STAGE, L"Component_Mesh_House5", L"Com_Mesh", (CComponent**)&m_pMeshCom)))
		return E_FAIL;
...
HRESULT CHouse5::SetUp_ConstantTable()
{
	if (nullptr == m_pShaderCom || 
...
HRESULT CHouse5::SetUp_OnTerrain()
{
	CManagement*	pManagement = CManagement::GetInstance();
	if (nullptr == pManagement)
		return -1;
...
CHouse5 * CHouse5::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CHouse5* pInstance = new CHouse5(pGraphic_Device);

	if (FAILED(pInstance->Ready_GameObject_Prototype()))
	{
		MSG_BOX("Failed while Creating CHouse5");
...
CGameObject * CHouse5::Clone_GameObject(void* pArg)
{
