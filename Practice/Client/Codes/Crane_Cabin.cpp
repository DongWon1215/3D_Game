...
#include "..\Headers\Crane_Cabin.h"

USING(Client)

CCrane_Cabin::CCrane_Cabin(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CGameObject(pGraphic_Device)
{

}

CCrane_Cabin::CCrane_Cabin(const CCrane_Cabin & rhs)
	: CGameObject(rhs)
{

}

HRESULT CCrane_Cabin::Ready_GameObject_Prototype() // 원본객체의 초기화.
{

	return S_OK;
}

HRESULT CCrane_Cabin::Ready_GameObject(void* pArg) // 복제된객체의 추가 초기화.
{
...
_int CCrane_Cabin::Update_GameObject(_double TimeDelta)
{
...
_int CCrane_Cabin::LateUpdate_GameObject(_double TimeDelta)
{
...
HRESULT CCrane_Cabin::Render_GameObject()
{
...
HRESULT CCrane_Cabin::Add_Component()
{
	// For.Com_Shader
	...
	// For.Com_Mesh
	if (FAILED(CGameObject::Add_Component(SCENE_STAGE, L"Component_Mesh_Crane_Cabin", L"Com_Mesh", (CComponent**)&m_pMeshCom)))
		return E_FAIL;
...
HRESULT CCrane_Cabin::SetUp_ConstantTable()
{
	if (nullptr == m_pShaderCom || 
...
HRESULT CCrane_Cabin::SetUp_OnTerrain()
{
	CManagement*	pManagement = CManagement::GetInstance();
	if (nullptr == pManagement)
		return -1;
...
CCrane_Cabin * CCrane_Cabin::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CCrane_Cabin* pInstance = new CCrane_Cabin(pGraphic_Device);

	if (FAILED(pInstance->Ready_GameObject_Prototype()))
	{
		MSG_BOX("Failed while Creating CCrane_Cabin");
...
CGameObject * CCrane_Cabin::Clone_GameObject(void* pArg)
{
	CCrane_Cabin* pInstance = new CCrane_Cabin(*this);
