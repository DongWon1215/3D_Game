...
#include "..\Headers\Burning_Car_2.h"

USING(Client)

CBurning_Car2::CBurning_Car2(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CGameObject(pGraphic_Device)
{

}

CBurning_Car2::CBurning_Car2(const CBurning_Car2 & rhs)
	: CGameObject(rhs)
{

}

HRESULT CBurning_Car2::Ready_GameObject_Prototype() // 원본객체의 초기화.
{

	return S_OK;
}

HRESULT CBurning_Car2::Ready_GameObject(void* pArg) // 복제된객체의 추가 초기화.
{
...
_int CBurning_Car2::Update_GameObject(_double TimeDelta)
{
...
_int CBurning_Car2::LateUpdate_GameObject(_double TimeDelta)
{
...
HRESULT CBurning_Car2::Render_GameObject()
{
...
HRESULT CBurning_Car2::Add_Component()
{
	// For.Com_Shader
	...
	// For.Com_Mesh
	if (FAILED(CGameObject::Add_Component(SCENE_STAGE, L"Component_Mesh_Burning_Car2", L"Com_Mesh", (CComponent**)&m_pMeshCom)))
		return E_FAIL;
...
HRESULT CBurning_Car2::SetUp_ConstantTable()
{
	if (nullptr == m_pShaderCom || 
...
HRESULT CBurning_Car2::SetUp_OnTerrain()
{
	CManagement*	pManagement = CManagement::GetInstance();
	if (nullptr == pManagement)
		return -1;
...
CBurning_Car2 * CBurning_Car2::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CBurning_Car2* pInstance = new CBurning_Car2(pGraphic_Device);

	if (FAILED(pInstance->Ready_GameObject_Prototype()))
	{
		MSG_BOX("Failed while Creating CBurning_Car2");
...
CGameObject * CBurning_Car2::Clone_GameObject(void* pArg)
{
	CBurning_Car2* pInstance = new CBurning_Car2(*this);
