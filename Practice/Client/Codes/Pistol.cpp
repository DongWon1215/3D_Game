...
CPistol::CPistol(LPDIRECT3DDEVICE9 pDevice)
	:CGameObject(pDevice)
{
}
...
HRESULT CPistol::Ready_GameObject_Prototype()
{
	return S_OK;
}

HRESULT CPistol::Ready_GameObject(void * pArg)
{
...
_int CPistol::Update_GameObject(_double TimeDelta)
{
...
_int CPistol::LateUpdate_GameObject(_double TimeDelta)
{
	if (m_eWeapon == WEAPON_PISTOL)
	{
		...
		if (pManagement->KeyPressing(KEY_LBUTTON) && m_iCurrentBullet > 0 && !m_bIsFrameStart && m_fCooltime <= 0.f)
		{
			...
			dynamic_cast<CPlayer*>(pManagement->Get_GameObject(L"GameObject_Player", SCENE_STAGE, L"Layer_Player", 0))->Get_PlayerState() == STATE_RELOAD && !m_bIsFrameStart || m_iCurrentBullet <= 0)
		{
...
HRESULT CPistol::Render_GameObject()
{
...
HRESULT CPistol::Add_Component()
{
	...
	// For.Com_Mesh
	if (FAILED(CGameObject::Add_Component(SCENE_STAGE, L"Component_Mesh_Pistol", L"Com_Mesh", (CComponent**)&m_pMeshCom)))
		return E_FAIL;
...
HRESULT CPistol::SetUp_ConstantTable()
{
	if (nullptr == m_pShaderCom ||
...
CPistol * CPistol::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CPistol* pInstance = new CPistol(pDevice);

	if (FAILED(pInstance->Ready_GameObject_Prototype()))
	{
		MSG_BOX("Failed while Creating CPistol");
...
CGameObject * CPistol::Clone_GameObject(void * pArg)
{
	CPistol* pInstance = new CPistol(*this);

	if (FAILED(pInstance->Ready_GameObject(pArg)))
	{
		MSG_BOX("Failed while Creating CPistol_Clone");
		Safe_Release(pInstance);
	}
