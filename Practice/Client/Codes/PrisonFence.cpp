...
#include "..\Headers\PrisonFence.h"

USING(Client)

CPrisonFence::CPrisonFence(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CGameObject(pGraphic_Device)
{

}

CPrisonFence::CPrisonFence(const CPrisonFence & rhs)
	: CGameObject(rhs)
{

}

HRESULT CPrisonFence::Ready_GameObject_Prototype() // 원본객체의 초기화.
{

	return S_OK;
}

HRESULT CPrisonFence::Ready_GameObject(void* pArg) // 복제된객체의 추가 초기화.
{
...
_int CPrisonFence::Update_GameObject(_double TimeDelta)
{
...
_int CPrisonFence::LateUpdate_GameObject(_double TimeDelta)
{
...
HRESULT CPrisonFence::Render_GameObject()
{
...
HRESULT CPrisonFence::Add_Component()
{
	// For.Com_Shader
	...
	// For.Com_Mesh
	if (FAILED(CGameObject::Add_Component(SCENE_STAGE, L"Component_Mesh_PrisonFence", L"Com_Mesh", (CComponent**)&m_pMeshCom)))
		return E_FAIL;
...
HRESULT CPrisonFence::SetUp_ConstantTable()
{
	if (nullptr == m_pShaderCom || 
...
HRESULT CPrisonFence::SetUp_OnTerrain()
{
	CManagement*	pManagement = CManagement::GetInstance();
	if (nullptr == pManagement)
		return -1;
...
CPrisonFence * CPrisonFence::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CPrisonFence* pInstance = new CPrisonFence(pGraphic_Device);

	if (FAILED(pInstance->Ready_GameObject_Prototype()))
	{
		MSG_BOX("Failed while Creating CPrisonFence");
...
CGameObject * CPrisonFence::Clone_GameObject(void* pArg)
{
	CPrisonFence* pInstance = new CPrisonFence(*this);
