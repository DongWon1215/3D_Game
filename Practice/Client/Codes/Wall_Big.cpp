...
#include "..\Headers\Wall_Big.h"

USING(Client)

CWall_Big::CWall_Big(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CGameObject(pGraphic_Device)
{

}

CWall_Big::CWall_Big(const CWall_Big & rhs)
	: CGameObject(rhs)
{

}

HRESULT CWall_Big::Ready_GameObject_Prototype() // 원본객체의 초기화.
{

	return S_OK;
}

HRESULT CWall_Big::Ready_GameObject(void* pArg) // 복제된객체의 추가 초기화.
{
...
_int CWall_Big::Update_GameObject(_double TimeDelta)
{
...
_int CWall_Big::LateUpdate_GameObject(_double TimeDelta)
{
...
HRESULT CWall_Big::Render_GameObject()
{
...
HRESULT CWall_Big::Add_Component()
{
	// For.Com_Shader
	...
	// For.Com_Mesh
	if (FAILED(CGameObject::Add_Component(SCENE_STAGE, L"Component_Mesh_Wall_Big", L"Com_Mesh", (CComponent**)&m_pMeshCom)))
		return E_FAIL;
...
HRESULT CWall_Big::SetUp_ConstantTable()
{
	if (nullptr == m_pShaderCom || 
...
HRESULT CWall_Big::SetUp_OnTerrain()
{
	CManagement*	pManagement = CManagement::GetInstance();
	if (nullptr == pManagement)
		return -1;
...
CWall_Big * CWall_Big::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CWall_Big* pInstance = new CWall_Big(pGraphic_Device);

	if (FAILED(pInstance->Ready_GameObject_Prototype()))
	{
		MSG_BOX("Failed while Creating CWall_Big");
...
CGameObject * CWall_Big::Clone_GameObject(void* pArg)
{
	CWall_Big* pInstance = new CWall_Big(*this);
