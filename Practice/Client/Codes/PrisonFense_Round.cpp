...
#include "..\Headers\PrisonFense_Round.h"

USING(Client)

CPrisonFense_Round::CPrisonFense_Round(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CGameObject(pGraphic_Device)
{

}

CPrisonFense_Round::CPrisonFense_Round(const CPrisonFense_Round & rhs)
	: CGameObject(rhs)
{

}

HRESULT CPrisonFense_Round::Ready_GameObject_Prototype() // 원본객체의 초기화.
{

	return S_OK;
}

HRESULT CPrisonFense_Round::Ready_GameObject(void* pArg) // 복제된객체의 추가 초기화.
{
...
_int CPrisonFense_Round::Update_GameObject(_double TimeDelta)
{
...
_int CPrisonFense_Round::LateUpdate_GameObject(_double TimeDelta)
{
...
HRESULT CPrisonFense_Round::Render_GameObject()
{
...
HRESULT CPrisonFense_Round::Add_Component()
{
	// For.Com_Shader
	...
	// For.Com_Mesh
	if (FAILED(CGameObject::Add_Component(SCENE_STAGE, L"Component_Mesh_PrisonFense_Round", L"Com_Mesh", (CComponent**)&m_pMeshCom)))
		return E_FAIL;
...
HRESULT CPrisonFense_Round::SetUp_ConstantTable()
{
	if (nullptr == m_pShaderCom || 
...
HRESULT CPrisonFense_Round::SetUp_OnTerrain()
{
	CManagement*	pManagement = CManagement::GetInstance();
	if (nullptr == pManagement)
		return -1;
...
CPrisonFense_Round * CPrisonFense_Round::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CPrisonFense_Round* pInstance = new CPrisonFense_Round(pGraphic_Device);

	if (FAILED(pInstance->Ready_GameObject_Prototype()))
	{
		MSG_BOX("Failed while Creating CPrisonFense_Round");
...
CGameObject * CPrisonFense_Round::Clone_GameObject(void* pArg)
{
	CPrisonFense_Round* pInstance = new CPrisonFense_Round(*this);
