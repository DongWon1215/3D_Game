...
#include "..\Headers\House6.h"

USING(Client)

CHouse6::CHouse6(LPDIRECT3DDEVICE9 pGraphic_Device)
...
HRESULT CHouse6::Ready_GameObject_Prototype() // 원본객체의 초기화.
{

	return S_OK;
}

HRESULT CHouse6::Ready_GameObject(void* pArg) // 복제된객체의 추가 초기화.
{
...
_int CHouse6::Update_GameObject(_double TimeDelta)
{
...
_int CHouse6::LateUpdate_GameObject(_double TimeDelta)
{
...
HRESULT CHouse6::Render_GameObject()
{
...
HRESULT CHouse6::Add_Component()
{
	...
	// For.Com_Mesh
	if (FAILED(CGameObject::Add_Component(SCENE_STAGE, L"Component_Mesh_House6", L"Com_Mesh", (CComponent**)&m_pMeshCom)))
		return E_FAIL;
...
HRESULT CHouse6::SetUp_ConstantTable()
{
	if (nullptr == m_pShaderCom || 
...
HRESULT CHouse6::SetUp_OnTerrain()
{
	CManagement*	pManagement = CManagement::GetInstance();
	if (nullptr == pManagement)
		return -1;
...
CHouse6 * CHouse6::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CHouse6* pInstance = new CHouse6(pGraphic_Device);

	if (FAILED(pInstance->Ready_GameObject_Prototype()))
	{
		MSG_BOX("Failed while Creating CHouse6");
...
CGameObject * CHouse6::Clone_GameObject(void* pArg)
{
