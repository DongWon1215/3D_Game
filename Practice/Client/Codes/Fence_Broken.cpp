...
#include "..\Headers\Fence_Broken.h"

USING(Client)

CFence_Broken::CFence_Broken(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CGameObject(pGraphic_Device)
{

}

CFence_Broken::CFence_Broken(const CFence_Broken & rhs)
	: CGameObject(rhs)
{

}

HRESULT CFence_Broken::Ready_GameObject_Prototype() // 원본객체의 초기화.
{

	return S_OK;
}

HRESULT CFence_Broken::Ready_GameObject(void* pArg) // 복제된객체의 추가 초기화.
{
...
_int CFence_Broken::Update_GameObject(_double TimeDelta)
{
...
_int CFence_Broken::LateUpdate_GameObject(_double TimeDelta)
{
...
HRESULT CFence_Broken::Render_GameObject()
{
...
HRESULT CFence_Broken::Add_Component()
{
	// For.Com_Shader
	...
	// For.Com_Mesh
	if (FAILED(CGameObject::Add_Component(SCENE_STAGE, L"Component_Mesh_Fence_Broken", L"Com_Mesh", (CComponent**)&m_pMeshCom)))
		return E_FAIL;
...
HRESULT CFence_Broken::SetUp_ConstantTable()
{
	if (nullptr == m_pShaderCom || 
...
HRESULT CFence_Broken::SetUp_OnTerrain()
{
	CManagement*	pManagement = CManagement::GetInstance();
	if (nullptr == pManagement)
		return -1;
...
CFence_Broken * CFence_Broken::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CFence_Broken* pInstance = new CFence_Broken(pGraphic_Device);

	if (FAILED(pInstance->Ready_GameObject_Prototype()))
	{
		MSG_BOX("Failed while Creating CFence_Broken");
...
CGameObject * CFence_Broken::Clone_GameObject(void* pArg)
{
	CFence_Broken* pInstance = new CFence_Broken(*this);
