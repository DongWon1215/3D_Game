                     ...
                     _In_ int       nCmdShow)
{
    ...
    while (true)
    {
		...
		if (TimerAcc > (1.0 / 60.0))
		{
			...
			pMainGame->Update_MainApp(pManagement->Compute_TimeDelta(L"Timer_60"));

			// 내 게임을 그리기위한 함수를 호출하자.
			pMainGame->Render_MainApp();
	...
	if (dwRefCnt = Safe_Release(pMainGame))
	{
		MSG_BOX("Failed while Release CMainGame");
		return FALSE;
	}
