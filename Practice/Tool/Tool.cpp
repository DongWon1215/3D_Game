...
BOOL CToolApp::InitInstance()
{
	...
	//HINSTANCE phInst = ::AfxGetInstanceHandle();
	//if (FAILED(pManagement->Ready_Input(phInst, m_hWnd)));
...
BOOL CToolApp::OnIdle(LONG lCount)
{
	...
	if (nullptr != pMainFrame->GetToolView())
	{
		pMainFrame->GetToolView()->UpdateInputDevice();

		pMainFrame->GetToolView()->m_pManagement->Update_Current_Scene(0.1f);
		
		pMainFrame->GetToolView()->Invalidate(false);
	}
